...
function localStorage_consomationtotal(addconso) {
    var maconso = localStorage.getItem('consomationtotal');
    if (maconso == null) {
        console.log('Creation de consomationtotal en localStorage');
        localStorage.setItem('consomationtotal',0);
    }
    var maconso = localStorage.getItem('consomationtotal');
    localStorage.setItem('consomationtotal',parseInt(maconso)+parseInt(addconso));
    return maconso
}

function remove_localStorage_consomationtotal(){
    localStorage.removeItem('consomationtotal');
}
...
function start(siteS,bdd){
    ...
    for (var cactegorie in virtuosite){
        conso7 = consoparcategorie0[cactegorie] + consoparcategorie1[cactegorie]*multiplicateur
...
//    priority: 2
//},
//function(id) { console.log("Last error:", browser.runtime.lastError); });
...
request.onsuccess = function() {
    ...
    db = request.result;
    //displayData(); 
    thelistesite = Promise.all([GetSite(0),GetSite(-1)]).then((results) => {
        sites = results
...
function deleteBDD() {
    var DBDeleteRequest = window.indexedDB.deleteDatabase("suivi_conso");
